...
def debug_decode():
    ...
    with torch.no_grad():
        for images, texts, text_lengths, raw_texts in val_loader:
            ...
            logits = model(images)
            print(f"logits形状: {logits.shape}")  # [T, B, V]
            ...
            # 检查每个样本的预测序列
            for b in range(logits.shape[1]):  # 遍历batch
                print(f"\n样本 {b}: '{raw_texts[b]}'")
                sample_indices = max_indices[:, b]  # [T]
                sample_probs = max_probs[:, b]      # [T]
                ...
                for idx in sample_indices:
                    idx_val = idx.item()
                    if idx_val != 0 and idx_val != prev_idx:  # 不是blank且不重复
